...
def main():
    ...
    # 游戏背景图片
    background_img = pygame.image.load("D:\\code\\python\\file\\lab5\\images\\bg.jpg")
    # 游戏棋盘
    ...
    #猫猫爆炸图
    boom_img=pygame.image.load("D:\\code\\python\\file\\lab5\\images\\boom.gif")
        ...
        if not game.show_win and game.AI_mode and game.get_player() == ai.team:
            ...
            for event in pygame.event.get():
                ...
                if not game.show_win:
                    ...
                    if game.quit_button.clicked_back(chessboard,event):
                        pygame.quit()
                        sys.exit()  # 退出程序

                    if game.defeat_button.clicked_back(chessboard,event):
                        # game.set_win(game.get_player())
                        game.set_win('b')
                    
                    if game.zhenghuo_button.clicked_back(chessboard,event):
                        screen.blit(boom_img, (110, 110))
                        break
