...
class ChessAI(object):
    ...
    def get_next_step(self, chessboard: ChessBoard):
        ...
        :该函数应当返回四个值 
            ...
            4 落子的纵坐标
        '''
        self.alpha_beta(1,-99999999,99999999,chessboard) #a往大取 b往小取 故初始值a为负无穷b为正无穷
        # if chessboard.chessboard_map[self.new_pos[0]][self.new_pos[1]]==None:
        #     chess_list=chessboard.get_chess()
        #     chess=chess_list[0]
        #     return chess.row,chess.col,chess.row,chess.col
        return self.old_pos[0],self.old_pos[1],self.new_pos[0],self.new_pos[1] #old_pos:要操作的 new_pos：落子
    ...
    def update(self,chessboard: ChessBoard,old_x,old_y,new_x,new_y,depth,a,b):
        temp=chessboard.chessboard_map[new_x][new_y]
        # temp=chessboard
        ...
        child=self.alpha_beta(depth+1,a,b,chessboard)  
        ...
        chessboard.chessboard_map[new_x][new_y]=temp
        # chessboard=temp
        # chessboard.chessboard_map[old_x][old_y].update_position(old_x,old_y)
        return child
    ...
    def alpha_beta(self, depth, a, b, chessboard: ChessBoard):
        if depth<self.max_depth:
            chess_list=chessboard.get_chess() #所有能走的棋子 chesslist的元素去看chess类
            for chess in chess_list:
                old_x=chess.row
                old_y=chess.col
                forward=chessboard.get_put_down_position(chess)
                #我方是红方 是max层 AI是黑方 是min层
                #max层 我方
                if depth%2==1 and self.team==chess.team: 
                    for new_x,new_y in forward:
                        child=self.update(chessboard,old_x,old_y,new_x,new_y,depth,a,b)
                        if (child>a or not self.old_pos)and depth==1: #如果是第一次落子 还未设置old pos：
                            self.old_pos=[chess.row,chess.col]
                            self.new_pos=[new_x,new_y]
                        a=max(a,child)
                        if a>=b:
                            return a #alpha剪枝时a改变 b固定
                        
                #min层 AI方
                elif depth%2==0 and self.team!=chess.team:
                    for new_x,new_y in forward:
                        child=self.update(chessboard,old_x,old_y,new_x,new_y,depth,a,b)
                        b=min(b,child)
                        if a>=b:
                            return b #beta剪枝时a固定 b改变
            ...
            return self.evaluate_class.evaluate(chessboard) #超过深度 在评估函数里找效益最好的返回        